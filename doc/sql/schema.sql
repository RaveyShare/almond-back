CREATE TABLE IF NOT EXISTS memory_items (
    ...
    tags JSON COMMENT '标签列表(JSON数组，元素为字符串标签)',
    item_type VARCHAR(50) DEFAULT 'general' COMMENT '记忆项类型(例如 general/qa/flashcard 等)',
    difficulty VARCHAR(20) DEFAULT 'medium' COMMENT '难度(例如 easy/medium/hard)',
    mastery INT NOT NULL DEFAULT 0 COMMENT '掌握度(数值，越大越掌握)',
    review_count INT NOT NULL DEFAULT 0 COMMENT '复习次数',
    review_date DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '最近复习时间',
    next_review_date DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '下次复习时间',
    ...
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='记忆项表';

CREATE TABLE IF NOT EXISTS review_schedules (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
    memory_item_id BIGINT NOT NULL COMMENT '记忆项ID',
    user_id BIGINT NOT NULL COMMENT '所属用户ID',
    ...
    INDEX idx_review_schedules_memory_item_id (memory_item_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='复习计划表';

CREATE TABLE IF NOT EXISTS memory_aids (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
    memory_item_id BIGINT COMMENT '关联的记忆项ID',
    ...
    INDEX idx_memory_aids_memory_item_id (memory_item_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='记忆辅助信息表';

CREATE TABLE IF NOT EXISTS shares (
    id VARCHAR(255) PRIMARY KEY COMMENT '分享ID',
    memory_item_id BIGINT NOT NULL COMMENT '被分享的记忆项ID',
    ...
    INDEX idx_shares_memory_item_id (memory_item_id),
